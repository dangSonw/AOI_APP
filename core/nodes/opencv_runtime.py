...
def _detections_from_components(mask: np.ndarray) -> list[dict[str, float | int]]:
    count, _, stats, centroids = cv2.connectedComponentsWithStats(
        (_uint8(mask) > 0).astype(np.uint8), connectivity=8,
    )
    return [
        {
            ...
            'centroidX': float(centroids[index, 0]),
            'centroidY': float(centroids[index, 1]),
        }
...
def _ecc_registration(
    ...
    cv2.findTransformECC(
        ...
        _gray(image).astype(np.float32) / 255.0,
        transform,
        model,
        (cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT, iterations, 1e-6),
    )
...
def execute_opencv_node(node_id: str, inputs: NodeInputs, parameters: NodeParameters) -> NodeOutputs:
    ...
    if node_id == 'resize':
        return {'processed-image': cv2.resize(
            image, (int(parameters['width']), int(parameters['height'])),
            interpolation=_interpolation(parameters['interpolation']),
        )}
    if node_id == 'color-conversion':
        conversions = {
            'bgr-to-gray': cv2.COLOR_BGR2GRAY, 'bgr-to-rgb': cv2.COLOR_BGR2RGB,
            'bgr-to-hsv': cv2.COLOR_BGR2HSV, 'rgb-to-gray': cv2.COLOR_RGB2GRAY,
        }
    ...
    if node_id == 'adaptive-threshold':
        mask = cv2.adaptiveThreshold(
            _gray(_uint8(image)), 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY,
            _odd(parameters['blockSize'], minimum=3), float(parameters['constant']),
        )
        return {'mask': mask}
    ...
    if node_id == 'hough-lines':
        ...
        detections = [] if lines is None else [
            {'x1': int(line[0][0]), 'y1': int(line[0][1]), 'x2': int(line[0][2]), 'y2': int(line[0][3])}
            for line in lines
        ]
        return {'detections': detections}
    if node_id == 'hough-circles':
        circles = cv2.HoughCircles(
            _gray(_uint8(image)), cv2.HOUGH_GRADIENT, 1.2, 10,
            minRadius=int(parameters['minimumRadius']), maxRadius=int(parameters['maximumRadius']),
        )
        detections = [] if circles is None else [
            {'centerX': int(round(x)), 'centerY': int(round(y)), 'radius': int(round(radius))}
    ...
    if node_id == 'ecc-registration':
        registered, transform = _ecc_registration(
            image, _image(inputs, 'reference'), str(parameters['motionModel']), int(parameters['iterations']),
        )
        return {'registered-image': registered, 'transform': transform}
    ...
    if node_id == 'rotate-image':
        ...
        rotated = cv2.warpAffine(
            image, transform, (width, height), flags=_interpolation(parameters['interpolation']),
            borderMode=_border_mode(parameters['borderMode']),
            borderValue=_color(parameters['borderValue'], name='Border value'),
        )
    ...
    if node_id == 'pad-image':
        ...
        padded = cv2.copyMakeBorder(
            image, top, bottom, left, right, _border_mode(parameters['borderMode']),
            value=_color(parameters['borderValue'], name='Border value'),
        )
    ...
    if node_id == 'warp-perspective':
        ...
        warped = cv2.warpPerspective(
            image, transform.astype(np.float64),
            (int(parameters['width']), int(parameters['height'])),
            flags=_interpolation(parameters['interpolation']),
            borderMode=_border_mode(parameters['borderMode']),
            borderValue=_color(parameters['borderValue'], name='Border value'),
        )
    ...
    if node_id == 'in-range-mask':
        ...
        converted = {
            'bgr': lambda value: value,
            'hsv': lambda value: cv2.cvtColor(value, cv2.COLOR_BGR2HSV),
            'lab': lambda value: cv2.cvtColor(value, cv2.COLOR_BGR2LAB),
            'grayscale': _gray,
        }[color_space](_uint8(image))
    ...
    if node_id == 'draw-contours':
        ...
        cv2.drawContours(
            canvas, contours, contour_index, _color(parameters['color'], name='Contour color'),
            int(parameters['thickness']), lineType=cv2.LINE_AA,
        )
        return {'annotated-image': canvas}
