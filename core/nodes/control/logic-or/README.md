...
`logic-or` performs **Logic OR** in an AOI pipeline. Evaluates variadic boolean inputs and routes true or false. Configure it in Node inspector and connect outputs to ports with matching data types.

**Use when:** you need a repeatable logic or step stored in a recipe and inspectable on its own.
...
Inputs are `values`:boolean. The node applies Logic OR. Outputs are `result`:boolean. Each key in the diagram is the exact port name used when connecting edges.

## How the algorithm works

- Validate input presence, data types, and shapes against `logic-or`.
- The node has no parameters; behavior is determined by its input and runtime contract.
- Apply **Logic OR**: Evaluates variadic boolean inputs and routes true or false.
...
**Goal:** Run logic or with correctly typed input (`values`:boolean) and inspect its output.

**Workflow:** `logic-not` → `logic-or`

- Drag **Logic OR** onto the canvas.
...
- Results depend on input and assumptions of Logic OR.
