...
`logic-and` performs **Logic AND** in an AOI pipeline. Evaluates variadic boolean inputs and routes true or false. Configure it in Node inspector and connect outputs to ports with matching data types.

**Use when:** you need a repeatable logic and step stored in a recipe and inspectable on its own.

**Quick flow:** `logic-not` → `logic-and` → `logic-or`
...
Inputs are `values`:boolean. The node applies Logic AND. Outputs are `result`:boolean. Each key in the diagram is the exact port name used when connecting edges.

## How the algorithm works

- Validate input presence, data types, and shapes against `logic-and`.
- The node has no parameters; behavior is determined by its input and runtime contract.
- Apply **Logic AND**: Evaluates variadic boolean inputs and routes true or false.
...
**Goal:** Run logic and with correctly typed input (`values`:boolean) and inspect its output.

**Workflow:** `logic-not` → `logic-and` → `logic-or`

- Drag **Logic AND** onto the canvas.
...
- Results depend on input and assumptions of Logic AND.
