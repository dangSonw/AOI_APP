...
def _image(inputs: NodeInputs, key: str = 'image') -> np.ndarray:
    image = _array(inputs, key)
    if image.ndim not in {2, 3}:
        raise ValueError(f'Input {key} must be a two- or three-dimensional image.')
    return image
...
def _border_mode(value: object) -> int:
    try:
        return {
            'constant': cv2.BORDER_CONSTANT, 'replicate': cv2.BORDER_REPLICATE,
            'reflect': cv2.BORDER_REFLECT_101, 'wrap': cv2.BORDER_WRAP,
        }[str(value)]
    except KeyError as error:
...
def execute_opencv_extended_node(node_id: str, inputs: NodeInputs, parameters: NodeParameters) -> NodeOutputs:
    if node_id == 'estimate-affine-transform':
        ...
        transform, _ = cv2.estimateAffine2D(
            source, destination, method=methods[method],
            ransacReprojThreshold=float(parameters['ransacThreshold']),
        )
    ...
    if node_id == 'warp-affine':
        ...
        return {'processed-image': cv2.warpAffine(
            image, transform, (width, height), flags=_interpolation(parameters['interpolation']),
            borderMode=_border_mode(parameters['borderMode']), borderValue=_border_value(parameters['borderValue']),
        )}
    ...
    if node_id == 'contour-metrics':
        ...
        for contour in _contours(inputs):
            ...
            metrics.append({
                'area': area,
                'perimeter': perimeter,
                'centroidX': float(moments['m10'] / moments['m00']) if moments['m00'] else 0.0,
                'centroidY': float(moments['m01'] / moments['m00']) if moments['m00'] else 0.0,
    ...
    if node_id in {'image-arithmetic', 'image-bitwise'}:
        ...
        operations = {
            'image-arithmetic': {'add': cv2.add, 'subtract': cv2.subtract, 'multiply': cv2.multiply},
            'image-bitwise': {'and': cv2.bitwise_and, 'or': cv2.bitwise_or, 'xor': cv2.bitwise_xor},
        }[node_id]
