...
from .errors import NodeExecutionContextRequired
from .models import NodeExecutionContext, NodeInputs, NodeOutputs, NodeParameters
...
def _normalized_cross_correlation(observed: np.ndarray, reference: np.ndarray) -> NodeOutputs:
    ...
    similarity = np.divide(covariance, denominator, out=np.zeros_like(covariance), where=denominator > 1e-8)
    equal_flat = (denominator <= 1e-8) & (np.abs(mean_first - mean_second) <= 1e-6)
...
def _median_mad(inputs: NodeInputs, parameters: NodeParameters, context: NodeExecutionContext) -> NodeOutputs:
    observed = _image(inputs.get('image'))
    arrays = _load_npz(
        context.read_artifact('golden-statistics', expected_media_types=(NUMPY_ARCHIVE_MEDIA_TYPE,)),
        name='Golden statistics',
    )
...
def _mahalanobis(inputs: NodeInputs, parameters: NodeParameters, context: NodeExecutionContext) -> NodeOutputs:
    observed = _image(inputs.get('image'))
    arrays = _load_npz(
        context.read_artifact('distribution-statistics', expected_media_types=(NUMPY_ARCHIVE_MEDIA_TYPE,)),
...
def _template_matching(inputs: NodeInputs, parameters: NodeParameters, context: NodeExecutionContext) -> NodeOutputs:
    ...
    template = _image(_load_npy(
        context.read_artifact('template-image', expected_media_types=(NUMPY_MEDIA_TYPE,)),
        name='Template image',
    ), name='Template image')
    ...
    detection = {
        ...
        'score': quality, 'label': 'template-match',
    }
    return {'detections': [detection], 'score': 1.0 - quality}


def execute_golden_with_context(
    node_id: str,
    inputs: NodeInputs,
...
def execute_binary_xor(inputs: NodeInputs) -> NodeOutputs:
    ...
    return {
        'difference-mask': difference.astype(np.uint8) * 255,
        'score': float(difference.mean()),
    }


def execute_score_fusion(inputs: NodeInputs, parameters: NodeParameters) -> NodeOutputs:
