...
`rotate-image` performs **Rotate image** in an AOI pipeline. Rotates and scales an image around its center with optional canvas expansion. Configure it in Node inspector and connect outputs to ports with matching data types.

**Use when:** you need a repeatable rotate image step stored in a recipe and inspectable on its own.

**Quick flow:** `image-input` → `rotate-image` → `image-output`
...
Inputs are `image`:image. The node applies OpenCV getRotationMatrix2D and warpAffine. Outputs are `processed-image`:image. Each key in the diagram is the exact port name used when connecting edges.

## How the algorithm works

- Validate input presence, data types, and shapes against `rotate-image`.
- Parameters `angleDegrees`, `scale`, `expandCanvas`, `interpolation`, `borderMode`, `borderValue` control processing; change one value at a time to trace its effect.
- Apply **OpenCV getRotationMatrix2D and warpAffine**: Rotates and scales an image around its center with optional canvas expansion.
...
- `processed-image` (`image`): Rotated image as `image`; preview it or connect a compatible downstream node.
...
**Goal:** Run rotate image with correctly typed input (`image`:image) and inspect its output.

**Workflow:** `image-input` → `rotate-image` → `image-output`

- Drag **Rotate image** onto the canvas.
...
- Results depend on input and assumptions of OpenCV getRotationMatrix2D and warpAffine.
