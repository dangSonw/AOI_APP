...
`sobel-gradient` performs **Sobel gradient** in an AOI pipeline. Computes a configured Sobel gradient. Configure it in Node inspector and connect outputs to ports with matching data types.

**Use when:** you need a repeatable sobel gradient step stored in a recipe and inspectable on its own.

**Quick flow:** `image-input` → `sobel-gradient` → `image-output`
...
Inputs are `image`:image. The node applies Sobel gradient. Outputs are `processed-image`:image. Each key in the diagram is the exact port name used when connecting edges.

## How the algorithm works

- Validate input presence, data types, and shapes against `sobel-gradient`.
- Parameters `dx`, `dy` control processing; change one value at a time to trace its effect.
- Apply **Sobel gradient**: Computes a configured Sobel gradient.
...
|---|---|---|---|---|---|---|
| `dx` | `integer` | `1` | `0` | `4` | — | Enter `integer` within Min/Max; try the default first. |
| `dy` | `integer` | `0` | `0` | `4` | — | Enter `integer` within Min/Max; try the default first. |

## Copy-ready usage example

**Goal:** Run sobel gradient with correctly typed input (`image`:image) and inspect its output.

**Workflow:** `image-input` → `sobel-gradient` → `image-output`

- Drag **Sobel gradient** onto the canvas.
...
- Results depend on input and assumptions of Sobel gradient.
