...
`clahe` performs **CLAHE** in an AOI pipeline. Runs contrast-limited adaptive histogram equalization. Configure it in Node inspector and connect outputs to ports with matching data types.

**Use when:** you need a repeatable clahe step stored in a recipe and inspectable on its own.

**Quick flow:** `image-input` → `clahe` → `image-output`
...
Inputs are `image`:image. The node applies CLAHE. Outputs are `processed-image`:image. Each key in the diagram is the exact port name used when connecting edges.

## How the algorithm works

- Validate input presence, data types, and shapes against `clahe`.
- Parameters `clipLimit`, `tileGridSize` control processing; change one value at a time to trace its effect.
- Apply **CLAHE**: Runs contrast-limited adaptive histogram equalization.
...
|---|---|---|---|---|---|---|
| `clipLimit` | `number` | `2.0` | `0.01` | `100.0` | — | Enter `number` within Min/Max; try the default first. |
| `tileGridSize` | `integer` | `8` | `1` | `256` | — | Enter `integer` within Min/Max; try the default first. |

## Copy-ready usage example

**Goal:** Run clahe with correctly typed input (`image`:image) and inspect its output.

**Workflow:** `image-input` → `clahe` → `image-output`

- Drag **CLAHE** onto the canvas.
...
- Results depend on input and assumptions of CLAHE.
