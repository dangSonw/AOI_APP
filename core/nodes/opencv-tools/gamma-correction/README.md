...
`gamma-correction` performs **Gamma correction** in an AOI pipeline. Applies a bounded 8-bit lookup-table gamma transform. Configure it in Node inspector and connect outputs to ports with matching data types.

**Use when:** you need a repeatable gamma correction step stored in a recipe and inspectable on its own.

**Quick flow:** `image-input` → `gamma-correction` → `image-output`
...
Inputs are `image`:image. The node applies OpenCV LUT. Outputs are `processed-image`:image. Each key in the diagram is the exact port name used when connecting edges.

## How the algorithm works

- Validate input presence, data types, and shapes against `gamma-correction`.
- Parameters `gamma` control processing; change one value at a time to trace its effect.
- Apply **OpenCV LUT**: Applies a bounded 8-bit lookup-table gamma transform.
...
- `processed-image` (`image`): Corrected image as `image`; preview it or connect a compatible downstream node.
...
**Goal:** Run gamma correction with correctly typed input (`image`:image) and inspect its output.

**Workflow:** `image-input` → `gamma-correction` → `image-output`

- Drag **Gamma correction** onto the canvas.
...
- Results depend on input and assumptions of OpenCV LUT.
