...
`warp-perspective` performs **Warp perspective** in an AOI pipeline. Applies a connected 3 by 3 perspective transform to an image. Configure it in Node inspector and connect outputs to ports with matching data types.

**Use when:** you need a repeatable warp perspective step stored in a recipe and inspectable on its own.

**Quick flow:** `image-input` → `warp-perspective` → `image-output`
...
Inputs are `image`:image, `transform`:transform. The node applies OpenCV warpPerspective. Outputs are `processed-image`:image. Each key in the diagram is the exact port name used when connecting edges.

## How the algorithm works

- Validate input presence, data types, and shapes against `warp-perspective`.
...
- Apply **OpenCV warpPerspective**: Applies a connected 3 by 3 perspective transform to an image.
...
**Goal:** Run warp perspective with correctly typed input (`image`:image, `transform`:transform) and inspect its output.

**Workflow:** `image-input` → `warp-perspective` → `image-output`

- Drag **Warp perspective** onto the canvas.
...
- Results depend on input and assumptions of OpenCV warpPerspective.
