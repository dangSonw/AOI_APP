...
`find-contours` performs **Find contours** in an AOI pipeline. Extracts vector contours from a mask. Configure it in Node inspector and connect outputs to ports with matching data types.

**Use when:** you need a repeatable find contours step stored in a recipe and inspectable on its own.

**Quick flow:** `global-threshold` → `find-contours` → `draw-contours`
...
Inputs are `mask`:mask. The node applies Find contours. Outputs are `contours`:contours. Each key in the diagram is the exact port name used when connecting edges.

## How the algorithm works

- Validate input presence, data types, and shapes against `find-contours`.
- Parameters `retrieval` control processing; change one value at a time to trace its effect.
- Apply **Find contours**: Extracts vector contours from a mask.
...
- `contours` (`contours`): Contours as `contours`; preview it or connect a compatible downstream node.

## How to enter parameters

| Key | Kind | Default | Min | Max | Options | How to enter / Meaning |
|---|---|---|---|---|---|---|
| `retrieval` | `select` | `external` | — | — | `external`, `list`, `tree` | Enter `select` within Min/Max; try the default first. |

## Copy-ready usage example

**Goal:** Run find contours with correctly typed input (`mask`:mask) and inspect its output.

**Workflow:** `global-threshold` → `find-contours` → `draw-contours`

- Drag **Find contours** onto the canvas.
...
**Expected output:** Produce contours with the declared type and no error.
...
- Results depend on input and assumptions of Find contours.
