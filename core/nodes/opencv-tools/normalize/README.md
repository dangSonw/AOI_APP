...
`normalize` performs **Normalize** in an AOI pipeline. Normalizes image intensity values. Configure it in Node inspector and connect outputs to ports with matching data types.

**Use when:** you need a repeatable normalize step stored in a recipe and inspectable on its own.

**Quick flow:** `image-input` → `normalize` → `image-output`
...
Inputs are `image`:image. The node applies Normalize. Outputs are `processed-image`:image. Each key in the diagram is the exact port name used when connecting edges.

## How the algorithm works

- Validate input presence, data types, and shapes against `normalize`.
- Parameters `alpha`, `beta` control processing; change one value at a time to trace its effect.
- Apply **Normalize**: Normalizes image intensity values.
...
|---|---|---|---|---|---|---|
| `alpha` | `number` | `0.0` | `-1000000.0` | `1000000.0` | — | Enter `number` within Min/Max; try the default first. |
| `beta` | `number` | `1.0` | `-1000000.0` | `1000000.0` | — | Enter `number` within Min/Max; try the default first. |

## Copy-ready usage example

**Goal:** Run normalize with correctly typed input (`image`:image) and inspect its output.

**Workflow:** `image-input` → `normalize` → `image-output`

- Drag **Normalize** onto the canvas.
...
- Results depend on input and assumptions of Normalize.
