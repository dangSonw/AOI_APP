...
`image-bitwise` performs **Image bitwise** in an AOI pipeline. Applies AND, OR, or XOR to equal-shaped integer images. Configure it in Node inspector and connect outputs to ports with matching data types.

**Use when:** you need a repeatable image bitwise step stored in a recipe and inspectable on its own.

**Quick flow:** `image-input` → `image-bitwise` → `image-output`
...
Inputs are `image`:image, `operand`:image. The node applies OpenCV bitwise operations. Outputs are `processed-image`:image. Each key in the diagram is the exact port name used when connecting edges.

## How the algorithm works

- Validate input presence, data types, and shapes against `image-bitwise`.
- Parameters `operation` control processing; change one value at a time to trace its effect.
- Apply **OpenCV bitwise operations**: Applies AND, OR, or XOR to equal-shaped integer images.
...
| Field | Value |
|---|---|
| Node ID | `image-bitwise` |
| Category | OpenCV tools |
| Status | `debug` |
| Execution target | `local-cpu` |
| Capabilities | `opencv`, `image-bitwise` |
...
|---|---|---|---|---|---|---|
| `operation` | `select` | `and` | — | — | `and`, `or`, `xor` | Bitwise image operation. |

## Copy-ready usage example

**Goal:** Run image bitwise with correctly typed input (`image`:image, `operand`:image) and inspect its output.

**Workflow:** `image-input` → `image-bitwise` → `image-output`

- Drag **Image bitwise** onto the canvas.
...
- Results depend on input and assumptions of OpenCV bitwise operations.
