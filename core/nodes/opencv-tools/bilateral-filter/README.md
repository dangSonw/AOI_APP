...
`bilateral-filter` performs **Bilateral filter** in an AOI pipeline. Smooths while preserving configured edges. Configure it in Node inspector and connect outputs to ports with matching data types.

**Use when:** you need a repeatable bilateral filter step stored in a recipe and inspectable on its own.

**Quick flow:** `image-input` → `bilateral-filter` → `image-output`
...
Inputs are `image`:image. The node applies Bilateral filter. Outputs are `processed-image`:image. Each key in the diagram is the exact port name used when connecting edges.

## How the algorithm works

- Validate input presence, data types, and shapes against `bilateral-filter`.
- Parameters `diameter`, `sigmaColor`, `sigmaSpace` control processing; change one value at a time to trace its effect.
- Apply **Bilateral filter**: Smooths while preserving configured edges.
...
|---|---|---|---|---|---|---|
| `diameter` | `integer` | `9` | `1` | `255` | — | Enter `integer` within Min/Max; try the default first. |
| `sigmaColor` | `number` | `75.0` | `0.0` | `10000.0` | — | Enter `number` within Min/Max; try the default first. |
| `sigmaSpace` | `number` | `75.0` | `0.0` | `10000.0` | — | Enter `number` within Min/Max; try the default first. |

## Copy-ready usage example

**Goal:** Run bilateral filter with correctly typed input (`image`:image) and inspect its output.

**Workflow:** `image-input` → `bilateral-filter` → `image-output`

- Drag **Bilateral filter** onto the canvas.
...
- Results depend on input and assumptions of Bilateral filter.
