...
`erode` performs **Erode** in an AOI pipeline. Erodes a spatial mask. Configure it in Node inspector and connect outputs to ports with matching data types.

**Use when:** you need a repeatable erode step stored in a recipe and inspectable on its own.

**Quick flow:** `global-threshold` → `erode` → `overlay-mask`
...
Inputs are `mask`:mask. The node applies Erode. Outputs are `processed-mask`:mask. Each key in the diagram is the exact port name used when connecting edges.

## How the algorithm works

- Validate input presence, data types, and shapes against `erode`.
- Parameters `iterations` control processing; change one value at a time to trace its effect.
- Apply **Erode**: Erodes a spatial mask.
...
**Goal:** Run erode with correctly typed input (`mask`:mask) and inspect its output.

**Workflow:** `global-threshold` → `erode` → `overlay-mask`

- Drag **Erode** onto the canvas.
...
- Results depend on input and assumptions of Erode.
