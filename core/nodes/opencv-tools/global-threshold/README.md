# Global threshold node

## Purpose and quick use

`global-threshold` performs **Global threshold** in an AOI pipeline. Applies a fixed global threshold. Configure it in Node inspector and connect outputs to ports with matching data types.

**Use when:** you need a repeatable global threshold step stored in a recipe and inspectable on its own.

**Quick flow:** `image-input` → `global-threshold` → `overlay-mask`
...
Inputs are `image`:image. The node applies Global threshold. Outputs are `mask`:mask. Each key in the diagram is the exact port name used when connecting edges.

## How the algorithm works

- Validate input presence, data types, and shapes against `global-threshold`.
- Parameters `threshold` control processing; change one value at a time to trace its effect.
- Apply **Global threshold**: Applies a fixed global threshold.
...
|---|---|---|---|---|---|---|
| `threshold` | `number` | `127.0` | `0.0` | `65535.0` | — | Enter `number` within Min/Max; try the default first. |

## Copy-ready usage example

**Goal:** Run global threshold with correctly typed input (`image`:image) and inspect its output.

**Workflow:** `image-input` → `global-threshold` → `overlay-mask`

- Drag **Global threshold** onto the canvas.
...
- Results depend on input and assumptions of Global threshold.
