...
`resize` performs **Resize** in an AOI pipeline. Resizes an image to configured dimensions. Configure it in Node inspector and connect outputs to ports with matching data types.

**Use when:** you need a repeatable resize step stored in a recipe and inspectable on its own.

**Quick flow:** `image-input` → `resize` → `image-output`
...
Inputs are `image`:image. The node applies Resize. Outputs are `processed-image`:image. Each key in the diagram is the exact port name used when connecting edges.

## How the algorithm works

- Validate input presence, data types, and shapes against `resize`.
- Parameters `width`, `height`, `interpolation` control processing; change one value at a time to trace its effect.
- Apply **Resize**: Resizes an image to configured dimensions.
...
|---|---|---|---|---|---|---|
| `width` | `integer` | `1024` | `1` | `16384` | — | Enter `integer` within Min/Max; try the default first. |
| `height` | `integer` | `1024` | `1` | `16384` | — | Enter `integer` within Min/Max; try the default first. |
| `interpolation` | `select` | `linear` | — | — | `nearest`, `linear`, `cubic`, `area` | Enter `select` within Min/Max; try the default first. |

## Copy-ready usage example

**Goal:** Run resize with correctly typed input (`image`:image) and inspect its output.

**Workflow:** `image-input` → `resize` → `image-output`

- Drag **Resize** onto the canvas.
...
- Results depend on input and assumptions of Resize.
