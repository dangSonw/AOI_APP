...
`median-blur` performs **Median blur** in an AOI pipeline. Applies median filtering. Configure it in Node inspector and connect outputs to ports with matching data types.

**Use when:** you need a repeatable median blur step stored in a recipe and inspectable on its own.

**Quick flow:** `image-input` → `median-blur` → `image-output`
...
Inputs are `image`:image. The node applies Median blur. Outputs are `processed-image`:image. Each key in the diagram is the exact port name used when connecting edges.

## How the algorithm works

- Validate input presence, data types, and shapes against `median-blur`.
- Parameters `kernelSize` control processing; change one value at a time to trace its effect.
- Apply **Median blur**: Applies median filtering.
...
**Goal:** Run median blur with correctly typed input (`image`:image) and inspect its output.

**Workflow:** `image-input` → `median-blur` → `image-output`

- Drag **Median blur** onto the canvas.
...
- Results depend on input and assumptions of Median blur.
