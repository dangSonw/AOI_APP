...
`blend-images` performs **Blend images** in an AOI pipeline. Alpha blends two images with identical dimensions, channels, and dtype. Configure it in Node inspector and connect outputs to ports with matching data types.

**Use when:** you need a repeatable blend images step stored in a recipe and inspectable on its own.

**Quick flow:** `image-input` → `blend-images` → `image-output`
...
Inputs are `image`:image, `overlay`:image. The node applies OpenCV addWeighted. Outputs are `processed-image`:image. Each key in the diagram is the exact port name used when connecting edges.

## How the algorithm works

- Validate input presence, data types, and shapes against `blend-images`.
- Parameters `alpha` control processing; change one value at a time to trace its effect.
- Apply **OpenCV addWeighted**: Alpha blends two images with identical dimensions, channels, and dtype.
...
2. Connect a `image` output to `overlay`. Provide `image` image data; verify shape, dtype, and channel order.

### Read outputs

- `processed-image` (`image`): Blended image as `image`; preview it or connect a compatible downstream node.
...
**Goal:** Run blend images with correctly typed input (`image`:image, `overlay`:image) and inspect its output.

**Workflow:** `image-input` → `blend-images` → `image-output`

- Drag **Blend images** onto the canvas.
...
**Example input:** Data for `image`:image, `overlay`:image; use uint8 BGR 640×480 for images and direct typed output from the shown source node for other types.
