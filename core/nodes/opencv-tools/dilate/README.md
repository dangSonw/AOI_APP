...
`dilate` performs **Dilate** in an AOI pipeline. Dilates a spatial mask. Configure it in Node inspector and connect outputs to ports with matching data types.

**Use when:** you need a repeatable dilate step stored in a recipe and inspectable on its own.

**Quick flow:** `global-threshold` → `dilate` → `overlay-mask`
...
Inputs are `mask`:mask. The node applies Dilate. Outputs are `processed-mask`:mask. Each key in the diagram is the exact port name used when connecting edges.

## How the algorithm works

- Validate input presence, data types, and shapes against `dilate`.
- Parameters `iterations` control processing; change one value at a time to trace its effect.
- Apply **Dilate**: Dilates a spatial mask.
...
|---|---|---|---|---|---|---|
| `iterations` | `integer` | `1` | `1` | `100` | — | Enter `integer` within Min/Max; try the default first. |

## Copy-ready usage example

**Goal:** Run dilate with correctly typed input (`mask`:mask) and inspect its output.

**Workflow:** `global-threshold` → `dilate` → `overlay-mask`

- Drag **Dilate** onto the canvas.
...
- Results depend on input and assumptions of Dilate.
