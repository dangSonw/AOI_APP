...
def execute_nearest_centroid(inputs: NodeInputs, parameters: NodeParameters) -> NodeOutputs:
    image = _image(inputs)
    ...
    model = NearestCentroid(metric=str(parameters['distanceMetric'])).fit(samples, targets)
    predicted = model.predict(queries)
    probabilities = np.zeros((len(predicted), len(classes)), dtype=np.float64)
    probabilities[np.arange(len(predicted)), predicted.astype(np.int64)] = 1.0
    return {'classified-detections': _classified(detections, probabilities, classes)}
...
def execute(inputs: NodeInputs, parameters: NodeParameters) -> NodeOutputs:
    return execute_nearest_centroid(inputs, parameters)
