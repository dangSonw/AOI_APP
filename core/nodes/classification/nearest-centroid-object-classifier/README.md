# Nearest-centroid object classifier node

## Purpose and quick use

`nearest-centroid-object-classifier` performs **Nearest-centroid object classifier** in an AOI pipeline. Classifies each detected object by the closest class-average BGR color. Configure it in Node inspector and connect outputs to ports with matching data types.

**Use when:** you need a repeatable nearest-centroid object classifier step stored in a recipe and inspectable on its own.

**Quick flow:** `image-input` → `global-threshold` → `connected-components` → `nearest-centroid-object-classifier` → `draw-detections`
...
Inputs are `image`:image, `detections`:detections. The node applies scikit-learn NearestCentroid. Outputs are `classified-detections`:detections. Each key in the diagram is the exact port name used when connecting edges.

## How the algorithm works

- Validate input presence, data types, and shapes against `nearest-centroid-object-classifier`.
- Parameters `implementation`, `distanceMetric`, `trainingSamples` control processing; change one value at a time to trace its effect.
- Compute one mean BGR centroid for each label.
- Measure each detected object color against every class centroid.
- Normalize inverse distances into classScores and select the highest score.
...
| Field | Value |
|---|---|
| Node ID | `nearest-centroid-object-classifier` |
| Category | Classification |
| Status | `debug` |
| Execution target | `local-cpu` |
| Capabilities | `numpy`, `scikit-learn`, `classification` |
...
**Goal:** Run nearest-centroid object classifier with correctly typed input (`image`:image, `detections`:detections) and inspect its output.

**Workflow:** `image-input` → `global-threshold` → `connected-components` → `nearest-centroid-object-classifier` → `draw-detections`

- Drag **Nearest-centroid object classifier** onto the canvas.
...
- Results depend on input and assumptions of scikit-learn NearestCentroid.
