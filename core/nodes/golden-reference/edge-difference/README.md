# Edge difference node

## Purpose and quick use

`edge-difference` performs **Edge difference** in an AOI pipeline. Compares edge evidence with a configured reference. Configure it in Node inspector and connect outputs to ports with matching data types.

**Use when:** you need a repeatable edge difference step stored in a recipe and inspectable on its own.

**Quick flow:** `image-input` → `edge-difference`
...
Inputs are `image`:image. The node applies Edge difference. Outputs are `anomaly-map`:anomaly-map, `score`:score. Each key in the diagram is the exact port name used when connecting edges.

## How the algorithm works

- Validate input presence, data types, and shapes against `edge-difference`.
- Parameters `threshold` control processing; change one value at a time to trace its effect.
- Apply **Edge difference**: Compares edge evidence with a configured reference.
...
|---|---|---|---|---|---|---|
| `threshold` | `number` | `0.2` | `0.0` | `1.0` | — | Enter `number` within Min/Max; try the default first. |

## Copy-ready usage example

**Goal:** Run edge difference with correctly typed input (`image`:image) and inspect its output.

**Workflow:** `image-input` → `edge-difference`

- Drag **Edge difference** onto the canvas.
...
- Results depend on input and assumptions of Edge difference.
