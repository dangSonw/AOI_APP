# Absolute difference node

## Purpose and quick use

`absolute-difference` performs **Absolute difference** in an AOI pipeline. Scores absolute deviation from configured normal reference. Configure it in Node inspector and connect outputs to ports with matching data types.

**Use when:** you need a repeatable absolute difference step stored in a recipe and inspectable on its own.

**Quick flow:** `image-input` → `absolute-difference`
...
Inputs are `image`:image. The node applies Absolute difference. Outputs are `anomaly-map`:anomaly-map, `score`:score. Each key in the diagram is the exact port name used when connecting edges.

## How the algorithm works

- Validate input presence, data types, and shapes against `absolute-difference`.
- Parameters `referenceAsset` control processing; change one value at a time to trace its effect.
- Apply **Absolute difference**: Scores absolute deviation from configured normal reference.
...
|---|---|---|---|---|---|---|
| `referenceAsset` | `text` | `golden` | — | — | — | Enter `text` within Min/Max; try the default first. |

## Copy-ready usage example

**Goal:** Run absolute difference with correctly typed input (`image`:image) and inspect its output.

**Workflow:** `image-input` → `absolute-difference`

- Drag **Absolute difference** onto the canvas.
...
- Results depend on input and assumptions of Absolute difference.
