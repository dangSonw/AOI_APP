...
`ssim` performs **SSIM** in an AOI pipeline. Scores structural similarity against a configured reference. Configure it in Node inspector and connect outputs to ports with matching data types.

**Use when:** you need a repeatable ssim step stored in a recipe and inspectable on its own.

**Quick flow:** `image-input` → `ssim`
...
Inputs are `image`:image. The node applies SSIM. Outputs are `anomaly-map`:anomaly-map, `score`:score. Each key in the diagram is the exact port name used when connecting edges.

## How the algorithm works

- Validate input presence, data types, and shapes against `ssim`.
- Parameters `windowSize` control processing; change one value at a time to trace its effect.
- Apply **SSIM**: Scores structural similarity against a configured reference.
...
|---|---|---|---|---|---|---|
| `windowSize` | `integer` | `11` | `3` | `255` | — | Enter `integer` within Min/Max; try the default first. |

## Copy-ready usage example

**Goal:** Run ssim with correctly typed input (`image`:image) and inspect its output.

**Workflow:** `image-input` → `ssim`

- Drag **SSIM** onto the canvas.
...
- Results depend on input and assumptions of SSIM.
