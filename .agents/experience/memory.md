...
## 22. Live Workflow Node State and Structured Logs

- Core workflow execution exposes an optional observer and remains independent from SQLAlchemy. Production inspection execution persists a `running` row before invoking each node, then updates that same activation row to `completed`, `faulted`, or `cancelled` with duration evidence.
- `inspection_node_runs.node_id` identifies the workflow node instance. Keep `algorithm_id` separate so repeated instances of one algorithm can be rendered and audited correctly.
- A node that emits the system `failure` control output is a faulted node even without an exception. Nodes not reached by the selected control branch remain visually idle.
- The Logs node selects exactly one destination: `popup`, `terminal`, or `file` (the `database` destination and its `workflow_logs` table were removed). Terminal messages use the `aoi.workflow` logger, which the `app.main` lifespan configures with a console handler at INFO so nothing is swallowed; file messages append durable `timestamp [LEVEL] run node algorithm: message` lines to `data/logs/workflow-log.txt` (override with the `workflow_log_file` setting); popup events are deduplicated by inspection run ID plus activation sequence.
- Use `python -m app.database.migrations upgrade` with `PYTHONPATH=backend` for local upgrades because this path reads the secured `.env` database URL; direct `alembic upgrade head` uses the password-free fallback URL from `alembic.ini`.
- Runtime colors, labels, and node duration belong to the Project dashboard's Inspection flow list, never the Workflow editor canvas. Persist evidence after a terminal run. When a run reaches `completed`, `faulted`, or `cancelled`, reset every Inspection flow runtime status to white but keep each node's last-run duration visible; a node duration only refreshes when that node executes again in a newer run.
